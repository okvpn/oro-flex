...
class RestActionReader
{
     ...
     */
    public function __construct(Reader $annotationReader, ParamReader $paramReader, $includeFormat = true, array $formats = [])
    {
    ...
    public function read(RestRouteCollection $collection, \ReflectionMethod $method, $resource)
    {
        ...
        foreach ($this->parents as $parent) {
            if (empty($parent) || str_ends_with($parent, '/')) {
    ...
    private function getHttpMethodAndResourcesFromMethod(\ReflectionMethod $method, $resource)
    {
        ...
        if (str_starts_with($httpMethod, self::COLLECTION_ROUTE_PREFIX)
        ...
        if ($isCollection && !empty($resource)) {
            $resourcePluralized = Inflector::pluralize(end($resource));
